 ...
 */
function safeWriteFile(filePath: string, newContent: string): { success: boolean; message: string } {
  try {
 ...
 */
function generateUnifiedDiff(original: string, modified: string, filename: string = 'grammar.g4'): string {
...
- **With load_imports=true**: Includes rules from all imported grammars`,
    inputSchema: {
      ...
      properties: {
        ...
        from_file: {
          type: 'string',
          description: 'Optional: path to a grammar file to read (overrides grammar_content). Recommended for file-based workflows.'
        },
        base_path: {
          type: 'string',
          description: 'Optional: base directory for resolving imports. Defaults to directory of from_file.'
        },
        load_imports: {
          type: 'boolean',
          description: 'Optional: if true, automatically load and merge imported grammars. Default: true.'
        },
        summary_only: {
          type: 'boolean',
          description: 'Optional: if true, returns only summary statistics without full rule details. Default: false. Use this for large multi-file grammars.'
        }
      },
...
Returns: List of issues with severity (error/warning/info), descriptions, line numbers, and affected rule names.`,
    inputSchema: {
      ...
      properties: {
        ...
        base_path: {
          type: 'string',
          description: 'Optional: base directory for resolving imports and tokenVocab. Required for multi-file grammars.'
        },
        load_imports: {
          type: 'boolean',
          description: 'Optional: if true, automatically load imported grammars and lexer vocabulary. Default: true.'
        },
        max_issues: {
          type: 'number',
          description: 'Optional: maximum number of issues to return. Default: 100. Use 0 for unlimited.'
        }
      },
      required: ['grammar_content']
    }
...
- Match count`,
    inputSchema: {
      ...
      properties: {
        ...
        rule_name: {
          type: 'string',
          description: 'Rule name or pattern. For wildcards: * matches any characters, ? matches single character. For regex: use standard regex syntax.'
        },
        from_file: {
          type: 'string',
          description: 'Optional: path to a grammar file to read'
        ...
        match_mode: {
          ...
          description: 'Matching mode: "exact" (default, exact name), "regex" (regex pattern), "wildcard" (* and ?), "partial" (substring search)'
        },
        use_regex: {
          type: 'boolean',
          description: 'DEPRECATED: Use match_mode="regex" instead. If true, treats rule_name as regex pattern.'
        }
      },
...
Returns: Modified grammar with new rule inserted, success message, position description, file write confirmation if applicable.`,
    inputSchema: {
      ...
      properties: {
        ...
        rule_name: {
          type: 'string',
          description: 'Name of the rule. UPPERCASE for lexer rules (ID, STRING), lowercase for parser rules (expression, term)'
        },
        pattern: {
          type: 'string',
          description: 'For lexer rules: The lexer pattern. Examples: [0-9]+, \\".*?\\", [a-zA-Z_][a-zA-Z0-9_]*'
        },
        definition: {
          type: 'string',
          description: 'For parser rules: The rule definition. Examples: "ID ASSIGN expr", "term (PLUS term)*"'
        },
        skip: {
          type: 'boolean',
        ...
        fragment: {
          type: 'boolean',
          description: 'Lexer only: If true, marks rule as fragment (reusable pattern, not a token)'
        },
        return_type: {
          type: 'string',
          description: 'Parser only: Return type specification (e.g., "String value", "int result")'
        },
        insert_after: {
          type: 'string',
          description: 'Optional: Insert this rule immediately after the specified rule name. Overrides alphabetical sorting.'
        },
        insert_before: {
          type: 'string',
          description: 'Optional: Insert this rule immediately before the specified rule name. Overrides alphabetical sorting.'
        },
        write_to_file: {
          type: 'boolean',
          description: 'If true, writes modified grammar back to from_file (requires from_file to be set)'
        },
        output_mode: {
          type: 'string',
          enum: ['full', 'diff'],
          description: 'Output format: "full" returns entire modified grammar (default), "diff" returns git-style unified diff'
        }
      },
      required: ['grammar_content', 'rule_name']
    }
...
Returns: Modified grammar with rule and all references renamed, update count, file write confirmation if applicable.`,
    inputSchema: {
      ...
      properties: {
        ...
        new_name: {
          type: 'string',
          description: 'New name for the rule (must follow ANTLR4 naming: uppercase for lexer, lowercase for parser)'
        },
        write_to_file: {
          type: 'boolean',
          description: 'If true, writes modified grammar back to from_file (requires from_file to be set)'
...
Returns: Modified grammar with fragment added, original pattern preserved in existing rules.`,
    inputSchema: {
      ...
      properties: {
        ...
        fragment_name: {
          type: 'string',
          description: 'Name for the fragment (must be UPPERCASE, e.g., DIGIT, LETTER, IDENTIFIER_PART)'
        },
        pattern: {
          type: 'string',
...
- File write confirmation if applicable`,
    inputSchema: {
      ...
      properties: {
        ...
        rules: {
          type: 'array',
          description: 'Array of lexer rules to add. Each rule requires name and pattern properties.',
          items: {
            type: 'object',
            properties: {
              name: {
...
- File write confirmation if applicable`,
    inputSchema: {
      ...
      properties: {
        ...
        rules: {
          type: 'array',
          description: 'Array of parser rules to add. Each rule requires name and definition properties.',
          items: {
            type: 'object',
            properties: {
              name: {
...
- File write confirmation if applicable`,
    inputSchema: {
      ...
      properties: {
        ...
        rules: {
          type: 'array',
          description: 'Array of mixed rules to add. Each rule requires type and name. Lexer rules need pattern, parser rules need definition.',
          items: {
            type: 'object',
            properties: {
              type: {
                type: 'string',
                enum: ['parser', 'lexer'],
                description: 'Rule type: "parser" for parsing rules (lowercase names), "lexer" for token rules (uppercase names)'
              },
              name: {
                type: 'string',
...
**Note:** This is a simplified simulation. For 100% accuracy, use ANTLR4 tooling.`,
    inputSchema: {
      ...
      properties: {
        ...
        show_positions: {
          type: 'boolean',
          description: 'If true, show detailed position information (line, column, start, end) for each token'
        },
        rules_to_test: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional: Test only specific lexer rules (by name). Useful for focused testing.'
        }
      },
...
- Summary of results`,
    inputSchema: {
      ...
      properties: {
        ...
        pattern: {
          type: 'string',
          description: 'Optional: Custom pattern template. Use {NAME} as placeholder for base name. Default: \'{NAME}\''
        },
        skip: {
          type: 'boolean',
...
- Summary of results`,
    inputSchema: {
      ...
      properties: {
        ...
        tokenize: {
          type: 'boolean',
          description: 'If true (default), splits input into individual tokens. If false, creates single token.'
        },
        prefix: {
          type: 'string',
...
- Mode indicator (🚀 Native or ⚠️ Simulation)`,
    inputSchema: {
      ...
      properties: {
        ...
        show_tree: {
          type: 'boolean',
          description: 'Optional: if true, displays the parse tree (native mode only). Default: false.'
        }
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        preserve_parentheses: {
          type: 'boolean',
          description: 'If true, always wrap inlined body in parentheses (safer but verbose). Default: auto-detect'
        },
        dry_run: {
          type: 'boolean',
...
- Statistics (total rules, strategy used)`,
    inputSchema: {
      ...
      properties: {
        ...
        anchor_rule: {
          type: 'string',
          description: 'For dependency strategy: the rule to use as anchor (rules used by this rule come first)'
        },
        parser_first: {
          type: 'boolean',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        rule_names: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional: Array of specific rule names to fix. If omitted, fixes all suspicious patterns'
        },
        dry_run: {
          type: 'boolean',
        ...
        output_mode: {
          type: 'string',
          enum: ['full', 'diff'],
          description: 'Output mode: "full" returns complete grammar, "diff" returns git-style diff (default: diff)'
        },
        write_to_file: {
          type: 'boolean',
          description: 'If true, writes modified grammar back to from_file (requires from_file to be set)'
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (typeof args === 'object' && args !== null) {
      ...
      const isPlaceholder = grammarContentStr && (
      ...
      if (grammarContentStr && typeof grammarContentStr === 'string' && grammarContentStr.trim() !== '' && !isPlaceholder) {
      ...
      const isPlaceholder2 = grammar2ContentStr && (
        /^\/\/\s*(read|from|file|placeholder)/i.test(grammar2ContentStr.trim()) ||
        grammar2ContentStr.trim().length < 50
      );
      
      if (grammar2ContentStr && typeof grammar2ContentStr === 'string' && grammar2ContentStr.trim() !== '' && !isPlaceholder2) {
...
Note: The update-rule, add-lexer-rule, and add-parser-rule tools automatically preserve this formatting style when making changes.`;
        ...
        case 'list-rules': {
          ...
          const text = rules.length === 0
            ? 'No rules found'
            : rules.map(rule => {
              const refs = rule.referencedRules.length > 0 ? ` → [${rule.referencedRules.join(', ')}]` : '';
        ...
        case 'add-rule': {
          ...
          if (isLexerRule) {
            ...
            result = AntlrAnalyzer.addLexerRule(
              grammarContent,
              ruleName,
              pattern,
              { skip, channel, fragment, insertAfter, insertBefore }
            );
          } else {
            // Parser rule
            if (!definition) {
            ...
            result = AntlrAnalyzer.addParserRule(
              grammarContent,
              ruleName,
              definition,
              { returnType, insertAfter, insertBefore }
            );
          }
        ...
        case 'update-rule': {
          ...
          const result = AntlrAnalyzer.updateRule(
            grammarContent,
            ruleName,
            newDefinition
          );
        ...
        case 'rule-statistics': {
          ...
          if (!stats) {
            return {
              content: [
                { type: 'text', text: `Rule '${ruleName}' not found` } as TextContent
              ],
              isError: true
            };
          }
        ...
        case 'merge-rules': {
          ...
          const result = AntlrAnalyzer.mergeRules(grammarContent, rule1Name, rule2Name, newRuleName);
        ...
        case 'add-rules': {
          ...
          const result = AntlrAnalyzer.addRules(
            ...
            rules.map(r => ({
              ...
              pattern: r.pattern,
              definition: r.definition,
              options: r.type === 'lexer'
                ? {
                  skip: r.skip,
                  channel: r.channel,
                  fragment: r.fragment
                }
                : r.return_type ? { returnType: r.return_type } : undefined
            }))
          );

          let output = `${result.summary}\n\nDetails:\n${result.results.map(r =>
            `${r.success ? '✓' : '✗'} ${r.name} (${r.type}): ${r.message}`
          ).join('\n')}\n\nModified grammar:\n${result.modified}`;
        ...
        case 'preview-tokens': {
          ...
          if (nativeAvailable) {
            ...
            if (nativeResult.success) {
              ...
              // Native runtime failed - fall back to simulation with explanation
              let output = `⚠️  Native ANTLR4 failed (falling back to simulation):\n`;
              output += nativeResult.errors.join('\n') + '\n\n';
          ...
          if (!nativeAvailable) {
            ...
            output += runtime.getInstallInstructions().split('\n').slice(0, 5).join('\n') + '\n...\n\n';
          } else {
...
${writeResult.message}`;
        ...
        case 'suggest-tokens-from-errors': {
          ...
          if (result.suggestions.length > 0) {
            ...
            output += 'No suggestions found. The error log may not contain recognizable token errors.';
          }
        ...
        case 'test-parser-rule': {
          ...
          let testGrammar = grammarContent;
          ...
          if (result.matched) {
            ...
            if (result.details?.actualTokens && result.details.actualTokens.length > 0) {
              output += `\nTokens matched:\n`;
              result.details.actualTokens.filter(t => !t.skipped).forEach(token => {
...
`;
        ...
        case 'sort-rules': {
          const strategy = (argsObj.strategy as 'alphabetical' | 'type' | 'dependency' | 'usage') || 'alphabetical';
...
`;
        ...
        case 'analyze-ambiguities': {
          const options = {
            checkIdenticalAlternatives: (argsObj.checkIdenticalAlternatives as boolean | undefined) ?? true,
            checkOverlappingPrefixes: (argsObj.checkOverlappingPrefixes as boolean | undefined) ?? true,
            checkAmbiguousOptionals: (argsObj.checkAmbiguousOptionals as boolean | undefined) ?? true,
