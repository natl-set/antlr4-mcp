...
export class Antlr4Runtime {
  ...
  constructor(config: RuntimeConfig = {}) {
    this.config = {
      ...
      javaPath: config.javaPath || process.env.JAVA_HOME ? `${process.env.JAVA_HOME}/bin/java` : 'java',
  ...
  async tokenize(
    ...
    if (!available) {
      return {
        success: false,
        tokens: [],
        errors: ['ANTLR4 runtime not available. Install Java and ANTLR4 for accurate tokenization.'],
        mode: 'simulation'
      };
    }
    ...
    try {
      ...
      const grammarName = options.grammarName || 
  ...
  private parseTokenOutput(output: string): Token[] {
    ...
    for (const line of lines) {
      ...
      const match = line.match(/@\d+,(\d+):(\d+)='([^']*)',<([^>]+)>,(\d+):(\d+)(?:,channel=(\w+))?/);
      if (match) {
        tokens.push({
          type: match[4],
          text: match[3],
